...
public class HotelParser {
    ...
    public boolean canBeParsed(String response) {
        ...
        return isNumberedListResponse(response) ||
               isConversationalHotelMention(response) ||
               parseCompactNumberedHotelList(response).size() > 0;
    }
    ...
    public List<HotelInfo> parse(String response) {
        ...
        // Prioritize parsing structured numbered lists with details
        if (isNumberedListResponse(response)) {
            List<HotelInfo> detailedList = parseNumberedHotelList(response);
            if (!detailedList.isEmpty()) {
                return detailedList;
            }
            // If it looked like a numbered list but didn’t match detailed format, try compact
            List<HotelInfo> compactList = parseCompactNumberedHotelList(response);
            if (!compactList.isEmpty()) {
                return compactList;
            }
        }

        // Try conversational style last
        if (isConversationalHotelMention(response)) {
    ...
    public List<HotelInfo> parseCompactNumberedHotelList(String response) {
        List<HotelInfo> hotels = new ArrayList<>();
        if (response == null || response.trim().isEmpty()) {
            return hotels;
        }
        ...
        Pattern pattern = Pattern.compile(
            "\\b\\d+\\.\\s*([A-Za-z0-9'&\\-\\s]+?(?:Hotel|Inn|Resort|Lodge|Suites|Boutique|Motel))\\b",
            Pattern.CASE_INSENSITIVE
        );
        ...
        while (matcher.find()) {
            String name = matcher.group(1).trim();
            hotels.add(new HotelInfo(name, 0, "", matcher.group(0).trim())); // Star rating & amenities not available
        }
        return hotels;
    }
